...
namespace NS.WEB.Modules.Profile
{
    public partial class NextDestinationsComponent
    {
        ...
        [Parameter] public RenderControlState<NextDestinations?> State { get; set; } = new(null, obj => obj == null || obj.Items.Empty());
        ...
        private async Task Delete(NextDestinationsEntry? entry, bool confirm = true)
        {
            try
            {
                ...
                if (confirmed)
                {
                    ...
                    await State.FinishLoading.Invoke(NextDestinations);
                }
            }
            catch (Exception ex)
            {
